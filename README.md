...
CREATE TABLE PUBLIC.REVIEW_COMMENT (
    ...
    REMOVED_AT TIMESTAMP DEFAULT NULL,
    CREATED_BY UUID NOT NULL DEFAULT AUTH.UID(),
    CONSTRAINT REVIEW_COMMENT_FKEY1 FOREIGN KEY (CREATED_BY) 
    REFERENCES PUBLIC.USERS(id) ON DELETE CASCADE,
    CONSTRAINT REVIEW_COMMENT_FKEY2 FOREIGN KEY (REVIEW_ID) 
    REFERENCES PUBLIC.REVIEW(id) ON DELETE CASCADE
) TABLESPACE PG_DEFAULT;

ALTER TABLE PUBLIC.REVIEW_COMMENT ENABLE ROW LEVEL SECURITY;

CREATE POLICY "PERMIT SELECT TO AUTHENTICATED" 
ON PUBLIC.REVIEW_COMMENT FOR SELECT USING (TRUE);

CREATE POLICY "PERMIT INSERT OWN DATA TO AUTHENTICATED" 
ON PUBLIC.REVIEW_COMMENT FOR INSERT TO AUTHENTICATED 
WITH CHECK (AUTH.UID() = CREATED_BY);

CREATE POLICY "PERMIT UPDATE OWN DATA TO AUTHENTICATED"  
ON PUBLIC.REVIEW_COMMENT FOR UPDATE TO AUTHENTICATED 
USING (AUTH.UID() = CREATED_BY);

CREATE POLICY "PERMIT DELETE OWN DATA TO AUTHENTICATED"  
ON PUBLIC.REVIEW_COMMENT FOR DELETE TO AUTHENTICATED 
USING (AUTH.UID() = CREATED_BY);

DROP TRIGGER IF EXISTS TRIGGER_UPDATED_AT ON PUBLIC.REVIEW_COMMENT;
CREATE TRIGGER TRIGGER_UPDATED_AT
BEFORE UPDATE ON PUBLIC.REVIEW_COMMENT
FOR EACH ROW
EXECUTE FUNCTION FUNC_UPDATED_AT();
```
